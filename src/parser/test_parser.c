...
#include "dwarfidlCLexer.h"
#include "dwarfidlCParser.h"
...
int main(int argc, char **argv)
{
	...
	pANTLR3_INPUT_STREAM in_fileobj = antlr3FileStreamNew((uint8_t *) filename,
        ANTLR3_ENC_UTF8);
	dwarfidlCLexer *lexer = dwarfidlCLexerNew(in_fileobj);
	CommonTokenStream *tokenStream = antlr3CommonTokenStreamSourceNew(
		ANTLR3_SIZE_HINT, TOKENSOURCE(lexer));
	dwarfidlCParser *parser = dwarfidlCParserNew(tokenStream); 
    dwarfidlCParser_toplevel_return ret = parser->toplevel(parser);
    Tree *tree = ret.tree;
