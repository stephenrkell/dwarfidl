...
STRING_LIT
	: (SINGLE_QUOTE
			(~SINGLE_QUOTE | ESCAPE_CHAR SINGLE_QUOTE)*
		SINGLE_QUOTE)
	| (DOUBLE_QUOTE
...
fp_clauses
	: OPEN_BRACE fp_clause* CLOSE_BRACE
		-> ^(FP_CLAUSES fp_clause+)	 
	;

fp_clause
	: expression SEMICOLON
		-> ^(FP_CLAUSE KEYWORD_RW expression)
	| (a=KEYWORD_R|a=KEYWORD_W|a=KEYWORD_RW) COLON expression SEMICOLON
		-> ^(FP_CLAUSE $a expression)
	;

loc_opcode
	: IDENT (OPEN INT (COMMA INT)? CLOSE)? SEMICOLON
		-> ^(OPCODE IDENT INT*)
	;

loc_list
	: OPEN_BRACE loc_opcode* CLOSE_BRACE
...
attr
	: FOOTPRINT EQUALS fp_clauses
		-> ^(ATTR FOOTPRINT fp_clauses)
	| attr_key EQUALS attr_value
		-> ^(ATTR attr_key attr_value);

attr_list
	: OPEN_SQUARE (attr (COMMA attr)*)? CLOSE_SQUARE
		-> attr+
	;
...
for_expression
	: (conditional_expression->conditional_expression)
		(KEYWORD_FOR identifier KEYWORD_IN conditional_expression
			-> ^(FP_FOR $for_expression identifier conditional_expression)
		)?
	;
...
logical_or_expression
	: (logical_and_expression
			-> logical_and_expression)
	  (KEYWORD_OR logical_and_expression
			-> ^(FP_OR $logical_or_expression logical_and_expression)
		)*
	;
...
logical_and_expression
	: (inclusive_or_expression
			-> inclusive_or_expression)
	  (KEYWORD_AND inclusive_or_expression
			-> ^(FP_AND $logical_and_expression inclusive_or_expression)
		)*
	;
...
inclusive_or_expression
	: (exclusive_or_expression
			-> exclusive_or_expression)
		('|' exclusive_or_expression
			-> ^(FP_BITOR $inclusive_or_expression exclusive_or_expression)
		)*
	;
...
exclusive_or_expression
	: (and_expression
			-> and_expression)
		('^' and_expression
			-> ^(FP_BITXOR $exclusive_or_expression and_expression)
		)*
	;
...
and_expression
	: (equality_expression
			-> equality_expression)
		('&' equality_expression
			-> ^(FP_BITAND $and_expression equality_expression)
		)*
	;
equality_expression
	: (relational_expression
			-> relational_expression)
		('==' relational_expression
			-> ^(FP_EQ $equality_expression relational_expression)
		|'!=' relational_expression
			-> ^(FP_NE $equality_expression relational_expression)
		)*
	;
...
relational_expression
	: (shift_expression
			-> shift_expression)
		('<' shift_expression
			-> ^(FP_LT $relational_expression shift_expression)
		|'>' shift_expression
			-> ^(FP_GT $relational_expression shift_expression) 
		|'<=' shift_expression
			-> ^(FP_LTE $relational_expression shift_expression)
		|'>=' shift_expression
			-> ^(FP_GTE $relational_expression shift_expression)
		)*
	;
...
shift_expression
	: (additive_expression
			-> additive_expression)
		('<<' additive_expression
			-> ^(FP_SHL $shift_expression additive_expression)
		|'>>' additive_expression
			-> ^(FP_SHR $shift_expression additive_expression)
		)*
	;
...
additive_expression
	: (multiplicative_expression
			-> multiplicative_expression)
		('+' multiplicative_expression
			-> ^(FP_ADD $additive_expression multiplicative_expression)
		|'-' multiplicative_expression
			-> ^(FP_SUB $additive_expression multiplicative_expression)
		)*
	;
...
multiplicative_expression
	: (unary_expression
			-> unary_expression)
		('*' unary_expression
			-> ^(FP_MUL $multiplicative_expression unary_expression)
		| '/' unary_expression
			-> ^(FP_DIV $multiplicative_expression unary_expression)
		| '%' unary_expression
			-> ^(FP_MOD $multiplicative_expression unary_expression)
		)*
;

unary_expression
	: '-' postfix_expression
		-> ^(FP_NEG postfix_expression)
	| '~' postfix_expression
		-> ^(FP_BITNOT postfix_expression)
	| '*' postfix_expression
...
postfix_expression
	:	(primary_expression->
		...
		|	OPEN args+=for_expression (COMMA args+=for_expression)* CLOSE
			-> ^(FP_APP $postfix_expression ^(FP_ARGS $args +))
		)*
	;

function_definition
	: name=identifier OPEN args+=identifier (COMMA args+=identifier)* CLOSE OPEN_BRACE expression CLOSE_BRACE
